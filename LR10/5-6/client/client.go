...
func main() {
	for {
		if sessionToken == "" {
			...
			fmt.Println("6. Получить права администратора")
			fmt.Println("7. Выход")
...
func adminAccess() {
	...
	var secret string
	fmt.Print("Введите секретный текст для получения прав администратора: ")
	fmt.Scan(&secret)

	credentials := map[string]string{"secret": secret}
	body, _ := json.Marshal(credentials)

	req, err := http.NewRequest("POST", apiBaseURL+"/admin-access", bytes.NewBuffer(body))
	if err != nil {
	...
	if err != nil {
		fmt.Printf("Ошибка при запросе прав администратора: %v\n", err)
		return
	}
	defer resp.Body.Close()
	...
	if resp.StatusCode == http.StatusOK {
		...
		if err != nil {
			fmt.Printf("Ошибка при обработке ответа сервера: %v\n", err)
			return
		}

		sessionToken = result["token"]

		fmt.Println("Вы получили права администратора!")
	} else {
		handleErrorResponse(resp)
	}
}
